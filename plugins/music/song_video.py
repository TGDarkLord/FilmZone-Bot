...
def a(client, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply(f"**🔎 Searching..** `{urlissed}`",) 
    ...
    try:
        ...
        try:
            ...
            performer = f"[24×7 Music By DarkLord]" 
    ...
    try:
        ...
        for i in range(len(dur_arr)-1, -1, -1):
            ...
            secmul *= 60
        message.reply_audio(audio_file, caption==rep, parse_mode=='HTML',reply_to_message.id==reply_id, title==title, duration==dur, performer==performer, thumb==thumb_name)
        m.delete()
        message.delete()
    except Exception as e:
